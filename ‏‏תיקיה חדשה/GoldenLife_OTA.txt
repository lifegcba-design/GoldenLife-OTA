...
void loadScenarioParams() {
    ...
    Serial.printf("🔁 מספר חזרות מרבי: %d\n", savedRepeat); // This is the value the user wants to see as total
        ...
        if (value.length() > 0) {
            ...
            if (command == 1) {
                ...
                if (value.length() >= 7) {
                    ...
                    scenarioRepeatMax = savedRepeat; // <-- ADDED/UPDATED THIS LINE
                    
                } else {
                    Serial.println("אין פרמטרים אופציונליים מ-BLE, מפעיל עם ערכים שמורים.");
                }
                // When starting via BLE, we are no longer just "ready"
...
class MyServerCallbacks : public BLEServerCallbacks {
    void onConnect(BLEServer *pServer) override {
        ...
        if (startStopChar) {
            uint8_t initialRemaining = 0; // Initialize to 0
            if (hasScenarioSaved()) {
                if (scenarioActive) {
                    ...
                    // If no scenario is active and not waiting for reactivation,
                    // and a scenario is saved, show the total saved repeats.
                    initialRemaining = savedRepeat;
                }
            } else {
                initialRemaining = 0; // No scenario saved, so 0 remaining
            }
...
class OtaControlCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) override {
        ...
        switch (command) {
            ...
            case 0x04: { // Get current OTA Status
...
void startScenario() {
    ...
    // --- חדש: עדכון BLE עם מספר התרחישים הנותרים לאחר התחלת תרחיש ---
    uint8_t currentRemaining = 0;
    if (scenarioRepeatMax > scenarioRepeatCount) {
        currentRemaining = scenarioRepeatMax - scenarioRepeatCount;
    }
    updateStartStopCharacteristic(1, savedDelay, savedRepeat, currentRemaining); // command 1 for active scenario
    // ------------------------------------------------------------------
}
...
void stopScenario(bool manualStop) {
    ...
    if (manualStop) {
        ...
        if (scenarioRepeatCount >= scenarioRepeatMax) {
            ...
            // --- חדש: עדכון BLE עם מספר התרחישים הנותרים לפני שינה בין חזרות ---
            uint8_t currentRemaining = 0;
            if (scenarioRepeatMax > scenarioRepeatCount) {
                currentRemaining = scenarioRepeatMax - scenarioRepeatCount;
            }
            updateStartStopCharacteristic(0, savedDelay, savedRepeat, currentRemaining); // Command 0 for status update
            // ------------------------------------------------------------------

            // Give BLE time to send notification before deep sleep
            delay(100); 
    ...
    if (currentButtonState == HIGH && (millis() - buttonPressStart > debounceDelay)) { // Use global debounceDelay
        if (!buttonHeld) {
            ...
            if (waitingForReactivation) {
                ...
                // Update BLE characteristic after state change
                updateStartStopCharacteristic(0, savedDelay, savedRepeat, savedRepeat);
...
void setup() {
    ...
    if (wakeupReason == ESP_SLEEP_WAKEUP_EXT1) {
        ...
        if (waitingForReactivation) {
            ...
            // Update BLE characteristic after state change
            updateStartStopCharacteristic(0, savedDelay, savedRepeat, 0); // 0 remaining as it's waiting for reactivation
            ...
            EEPROM.commit();
            // Update BLE characteristic after state change
            updateStartStopCharacteristic(0, savedDelay, savedRepeat, 0); // 0 remaining as all completed
            enterDeepSleep(true);
        }
    }
    ...
    loadSettings();
    // After loading scenario parameters, ensure scenarioRepeatMax is set
    scenarioRepeatMax = savedRepeat; // This is crucial for initial display

    if (startStopChar) {
        uint8_t initialRemaining = 0;
        if (hasScenarioSaved()) {
            // If a scenario is saved, the initial remaining should be the total savedRepeat
            // unless a scenario is active or waiting for reactivation.
            if (scenarioActive) {
                if (scenarioRepeatMax > scenarioRepeatCount) {
                    initialRemaining = scenarioRepeatMax - scenarioRepeatCount;
                } else {
